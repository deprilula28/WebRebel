...
function parseDOMElementScript(node){
    ...
    else if(source.indexOf("/") == -1 || (source.indexOf(/[^.]*\.[^.]{2,3}(?:\.[^.]{2,3})?$/) > source.indexOf("/"))) subscriptionRelPath = source;
    ...
    console.log(source, subscriptionRelPath);
...
function serverCodeUpdate(json){
    ...
    var script = subscribedScripts[json.source];
    if(!script){
        console.warn("Detected code change for script not subscribed to");
        return;
    }
    
    var lines = script.innerHTML.split(/(?<!\\)(?>\\\\)*\\q/); //Thanks to http://stackoverflow.com/a/56671
